...
import { LayoutDashboard, Package, Clock, FolderOpen, Users } from 'lucide-react';
...
export default async function ProductsPage({
  ...
  const [approvalData, allProductsData, categoriesData, sellersData] = await Promise.all([
    activeTab === 'approval' ? getApprovalQueue(params.search, params.seller) : null,
    activeTab === 'all' ? getAllProducts(params.search, params.status, params.seller) : null,
  ...
  const activeProducts = allProductsData?.products?.filter(p => p.is_available && !p.is_suspended && !p.admin_suspended).length || 0;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-purple-50/30">
      <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {TABS.map((tab, index) => {
              ...
              return (
                <a
                  key={tab.id}
                  href={`?tab=${tab.id}`}
                  className={`relative p-6 flex items-center gap-4 transition-all duration-300 group
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <p className={`font-semibold ${isActive ? tab.textColor : 'text-gray-700'}`}>
                        {tab.label}
                      </p>
                      {count > 0 && (
        ...
        <Suspense fallback={<LoadingState />}>
          {activeTab === 'approval' && approvalData && (
            <ApprovalQueue 
              data={approvalData} 
          ...
          {activeTab === 'categories' && categoriesData && (
            <CategoryManager data={categoriesData} />
          )}
        </Suspense>
      </div>
    </div>
